...
export async function activate(context: vscode.ExtensionContext) {
    ...
    let disposable = vscode.commands.registerCommand('apiHub.certification.validateRepository', (apiModule?: ValidationModuleType) => {
        ...
        vscode.window.withProgress(
            ...
            async (progress, token) => {
                ...
                serviceUrl = vscode.workspace.getConfiguration('apiHub.certification.service').get<string>('url');
                if (!serviceUrl || serviceUrl.trim().length === 0) {
                    vscode.window.showErrorMessage('Certification service URL is not configured', 'Open Settings').then(item => {
                        if (item === 'Open Settings') {
                            vscode.commands.executeCommand('workbench.action.openSettings', 'apiHub.certification.service.url');
                        }
                    });
    ...
    statusBarItem.text = 'APIHub Certification';
    statusBarItem.tooltip = 'Validate local APIs & Open APIHub Application';
    statusBarItem.color = '#00ff00';
    statusBarItem.command = {
        command: 'apiHub.certification.validateRepository',
        title: 'Validate local APIs & Open APIHub Application',
        tooltip: 'Validate local APIs & Open APIHub Application',
    };
    statusBarItem.show();

    const statusBarFile = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 2);
    statusBarFile.text = 'APIHub File Lint';
    statusBarFile.tooltip = 'Open APIHub Application File & Validate API file with a ruleset';
    statusBarFile.color = '#00ff00';
    statusBarFile.command = {
        command: 'apiHub.certification.getValidationRules',
    ...
    let getValidationRules = vscode.commands.registerCommand('apiHub.certification.getValidationRules', () => {
        if(!fileTriggerActivate){
            certificationFrontEnd = new CertificationServiceWebView(context, new HubController());
    ...
    let validateFile = vscode.commands.registerCommand('apiHub.certification.validateFile', (response: ValidationFileModuleType) => {
        serviceUrl = vscode.workspace.getConfiguration('apiHub.certification.service').get<string>('url');
        if (!serviceUrl || serviceUrl.trim().length === 0) {
            vscode.window.showErrorMessage('Certification service URL is not configured', 'Open Settings').then(item => {
                if (item === 'Open Settings') {
