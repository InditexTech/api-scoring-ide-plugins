...
const enum SpecialMappingKeys {
    Null = '~',
    MergeKey = '<<',
}

/** Returns a Location of a JsonPath in a parsed document
 * 
 * @param ast
 * @param lineMap
 * @param metadata 
 ...
 * @returns 
 */
export const getLocationForJsonPath: GetLocationForJsonPath<YamlParserResult<unknown>> = ({ ast, lineMap, metadata }, path, closest = false) => {
...
/** Returns the parent Location of a JsonPath in a parsed document
 * 
 * @param ast
 * @param lineMap
 * @param metadata 
 ...
 * @returns 
 */
export const getOuterLocationForJsonPath: GetLocationForJsonPath<YamlParserResult<unknown>> = ({ ast, lineMap, metadata }, path, closest = false) => {
    let node = findNodeAtPath(ast, path, {
        closest,
        mergeKeys: metadata !== undefined && metadata.mergeKeys === true,
    });
    if (node === void 0) return;
...
export const getKeyLocationForJsonPath: GetLocationForJsonPath<YamlParserResult<unknown>> = ({ ast, lineMap, metadata }, path, closest = false) => {
    let node = findNodeAtPath(ast, path, {
        closest,
        mergeKeys: metadata !== undefined && metadata.mergeKeys === true,
    });
...
function findNodeAtPath(node: YAMLNode, path: JsonPath, { closest, mergeKeys }: { closest: boolean; mergeKeys: boolean }) {
    pathLoop: for (const segment of path) {
        if (!isObject(node)) {
            return checkIfClosest(closest, node);
        }
...
function checkIfClosest(closest: boolean, node: YAMLNode) {
