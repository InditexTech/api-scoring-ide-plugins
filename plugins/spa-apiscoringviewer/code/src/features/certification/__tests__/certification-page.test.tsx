...
const apiMock = jest.mocked(api, true);
...
describe("renders inside IDE", () => {
  ...
  test("renders certification", async () => {
    act(() => {
      render(<CertificationPage />, { wrapper: Providers });
    });
    ...
    const currentTabContainer = within(screen.getByTestId("ApiValidation-API Sample-1"));

    postMessage.mockReset();

    const revalidateLinterButton = screen.getByTestId("RevalidateButton-API Sample-1");
    ...
    const secondTabContainer = within(screen.getByTestId("ApiValidation-Other API-1"));
    ...
    expect(postMessage).toBeCalledWith(
      {
        ...
        payload: {
          ...
          infoPosition: { char: 14, lastLine: undefined, lastchar: undefined, line: 13 },
        },
      },
      "*",
    );
  ...
  test("api validation results are updated", async () => {
    ...
    // New errors are displayed
    const tabContainer = within(await screen.findByTestId("ApiValidation-API Sample-1"));
    ...
    const securityValidationResult = tabContainer.getByTestId("ValidationResult-3");
    expect(
    ...
    const documentationValidationResult = tabContainer.getByTestId("ValidationResult-2");
    expect(
      within(documentationValidationResult).getByTestId("Issue-README.md-13-MD034-no-bare-urls"),
    ).toBeInTheDocument();
  });
  ...
  test("revalidation module error is displayed and reseted", async () => {
    ...
    const runLintingButton = screen.getByTestId("RevalidateButton-API Sample-1");
    await userEvent.click(runLintingButton);

    const revalidateButton = expect(runLintingButton).toHaveAttribute("data-loading", "true");
...
describe("renders inside browser", () => {
  beforeEach(() => {
    jest.spyOn(Router, "useParams").mockReturnValue({ api: "API Sample", protocol: "REST" });
    isVsCodeMock.mockReturnValue(false);
  });

  test("renders data from network provider", async () => {
    apiMock.validations.latest.mockResolvedValue(CERTS_PAYLOAD.results[0]);
  ...
  test("shows error when network error", async () => {
    const networkError = new ResponseError("invalid request");
    apiMock.validations.latest.mockRejectedValue(networkError);

    act(() => {
      render(<CertificationPage />, { wrapper: Providers });
    });

    expect(await screen.findByTestId("CertificationPage-NetworkError")).toBeInTheDocument();
  });
});
...
async function checkApiSampleContent() {
  ...
  expect(screen.getByTestId("ApiHeading-Badge-API Sample-1")).toHaveTextContent("REST");
  expect(screen.getByTestId("ApiHeading-Score-API Sample-1")).toHaveTextContent("D");

  expect(tabContainer.getByTestId("AccordionControlTitle-1")).toBeInTheDocument();
  expect(tabContainer.getByTestId("AccordionControlTitle-3")).toBeInTheDocument();
  expect(tabContainer.getByTestId("AccordionControlTitle-2")).toBeInTheDocument();
